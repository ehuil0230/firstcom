...
const postData = function(req) {
    let showData= ['一类项目申请量','二类项目申请量','项目申请总量'];
    ...
    for (let i = 0; i < data.length-1; i++) {
        for (let j = 0; j < year.length; j++) {
            data[i].data.push(Random.natural(1,1000));
        }
    }
    for (let i = 0; i < data.length-1; i++) {
        for (let j = 0; j < year.length; j++) {
            data[data.length-1].data.push(parseInt(data[i].data[j] + data[i+1].data[j]));
        }
    }
...
Mock.mock(`${domain}/getxiangmushenbaoData`, /get|post/i, postData); // 定义请求链接，类型，还有返回数据
...
Mock.mock(`${domain}/getzhishishenbaoData`, /get|post/i, postData2); // 定义请求链接，类型，还有返回数据
