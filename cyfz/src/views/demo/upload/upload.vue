<template>
    <div class="myupload">
        <el-upload
            ...
            <el-button v-if="autoUp" style="margin-left: 10px;" size="small" type="success" @click="submitUpload"
                       icon="el-icon-upload">
                上传文件
            </el-button>
            <el-button style="margin-left: 10px;" size="small" type="success" @click="downloadAll"
...
<script>
    ...
    /*function fileDownLoadBlob(data) {//blob对文件进行下载
        let linkElement = document.createElement('a')//创建点击下载的元素
        //判断浏览器是否支持blob对象
        let blob
        try {
            ...
            if (e.name == 'TypeError' && window.BlobBuilder) {
                ...
                blob = blobbuilder.getBlob('text/plain')
            } else {
        ...
        linkElement.setAttribute('downLoad', (!!data.zipName) ? data.zipName : 'downLoad')
        linkElement.click()
    }*/
    ...
    export default {
        ...
        methods: {
                    ...
                    if (err) {
                        ...
                        this.$axios({//校验md5
                            ...
                            if (data.msg === '正常上传') {//校验通过，附件上传
                                ...
                                this.fileList.splice(this.fileList.length, 0, obj)
                            }
                        }).catch(function (error) {
                            console.log(error)
                        })
            ...
            toUploadSuccess(response, file, fileList) {
                /*if(!!this.$parent.onsuccess(response, file, fileList)){
                  this.$parent.onsuccess(response, file, fileList);
                }*/
            ...
            handeldownloadFile(Params) {
                ...
                this.$axios({
                    ...
                    fileDownload(res, this.currentFileData.response.name);
                }).catch(function (error) {
                    // console.log(error)
                    that.$message({
                        type: 'info',
