<template>
    <div class="rightMain">
        ...
        <!--工具栏-新建-->
        <createbar key="jbjggs" v-if="xjDialogVisible" :taVisible="xjDialogVisible" :dialogData="dialogData"
    ...
    export default {
        ...
        data() {
            return {
                ...
                controlSatrtTime: { // 结束时间大于开始时间
                    disabledDate: time => {
                        if (this.searchParams.startTime !== '') {
                            return time.getTime() < new Date(this.searchParams.startTime).getTime() - 8.64e7;
                        }
                    }
                },
                controlEndTime: { // 结束时间大于开始时间
                    disabledDate: time => {
                ...
                //调用组件传输返回页面数据
                getDataWay: {dataType: "url", data: this.$api.xtglApi.xtglTzggQuery, type: 'post',param:{proclamationDm:'0006'}},
            }
        },
        methods: {
            //状态初始化
            ...
            toolbarClick(item, event) {
                ...
                if (item.title == '新建') {
                    ...
                    that.dialogData.fromType='0006';
                    that.dialogData.spType = '010006';
                }
                if (item.title == '修改') {
        ...
        created() {
            ...
            /*var that = this
            this.$$request({
                url: "http://mockjs.com/api/jbjggs",
